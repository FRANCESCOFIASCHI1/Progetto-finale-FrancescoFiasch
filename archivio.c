...
void *caposc_body(void *arg) {
  ...
  while (continua) {
    while (true){
      ...
      char *appoggio = strdup(string);
      
      xsem_wait(a->sem_caposc_free,__LINE__,__FILE__);
      a->buffer[*(a->caposcindex) % PC_buffer_len] = appoggio;
      *(a->caposcindex) += 1;
...
void *writer_body(void *arg) {
  ...
  while(true) {
    ...
    string = a->buffer[*(a->scrittoindex) % PC_buffer_len];
...
void *capolet_body(void *arg) {
  ...
  while(continua) {
    while (true){
      ...
      a->buffer[*(a->capoletindex) % PC_buffer_len] = appoggio;
      *(a->capoletindex) += 1;
...
void *reader_body(void *arg) {
  ...
  while(true) {
    ...
    string = a->buffer[*(a->lettoindex) % PC_buffer_len];
